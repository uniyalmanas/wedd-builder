    ...
    <form onSubmit={submit} className="space-y-8">
      <div className="grid md:grid-cols-2 gap-8">
        <div>
          ...
          <input name="name" value={form.name} onChange={handle} required placeholder="Your name" className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Phone Number *</label>
          <input name="phone" value={form.phone} onChange={handle} required placeholder="+91 98765 43210" className="input-field" />
        </div>
        <div>
        ...
        <div>
          ...
          <input name="date" type="date" value={form.date} onChange={handle} required className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Event City *</label>
          <input name="city" value={form.city} onChange={handle} required placeholder="e.g. Dehradun, Jaipur" className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Service Needed *</label>
          <select name="service" value={form.service} onChange={handle} required className="input-field bg-transparent">
            <option value="">Select a service</option>
            <option>Full-Service Planning</option>
            <option>Theme & Floral Decor</option>
            <option>Month-of Coordination</option>
            <option>Destination & Logistics</option>
            <option>Entertainment & Artist Booking</option>
            <option>Catering & Menu Design</option>
      ...
      <div>
        ...
        <select name="budget" value={form.budget} onChange={handle} className="input-field bg-transparent">
    ...
    <form onSubmit={submit} className="space-y-8">
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <input name="partnerName" value={form.partnerName} onChange={handle} placeholder="Partner's name (if wedding)" className="input-field" />
        </div>
        <div>
        ...
        <div>
          ...
          <input name="email" type="email" value={form.email} onChange={handle} required placeholder="your@email.com" className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Event Date *</label>
          <input name="eventDate" type="date" value={form.eventDate} onChange={handle} required className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Event Type *</label>
          <select name="eventType" value={form.eventType} onChange={handle} required className="input-field bg-transparent">
            <option value="">Select event type</option>
            <option>Hindu Wedding</option>
            <option>Muslim Wedding</option>
            <option>Christian Wedding</option>
            <option>Sikh Wedding</option>
            <option>Inter-religion Wedding</option>
        ...
        <div>
          ...
          <input name="venue" value={form.venue} onChange={handle} placeholder="Venue or hotel name" className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Event City *</label>
          <input name="city" value={form.city} onChange={handle} required placeholder="City, State" className="input-field" />
        </div>
        <div>
          <label className="section-label block mb-2">Expected Guests</label>
          <select name="guestCount" value={form.guestCount} onChange={handle} className="input-field bg-transparent">
        ...
        <div>
          ...
          <select name="duration" value={form.duration} onChange={handle} className="input-field bg-transparent">
            <option value="">Select duration</option>
            <option>2–4 Hours</option>
            <option>4–6 Hours</option>
            <option>Full Day (8–10 hrs)</option>
            <option>Multi-Day (2 days)</option>
            ...
            <label key={pkg} className="flex items-center gap-3 cursor-pointer group">
              <div
                ...
                className={`w-5 h-5 border flex items-center justify-center flex-shrink-0 transition-colors duration-200 ${
                  form.packages.includes(pkg) ? 'bg-gold-500 border-gold-500' : 'border-charcoal-400'
                }`}
              >
                {form.packages.includes(pkg) && <span className="text-white text-xs">✓</span>}
              </div>
              <span className="font-sans text-sm text-charcoal-700">{pkg}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <select name="budget" value={form.budget} onChange={handle} required className="input-field bg-transparent">
          <option value="">Select budget range</option>
      ...
      <div>
        ...
        <select name="referral" value={form.referral} onChange={handle} className="input-field bg-transparent">
          <option value="">Select source</option>
          <option>Instagram</option>
          <option>Google Search</option>
          <option>Friend / Family Referral</option>
          <option>JustDial</option>
          <option>WedMeGood / WeddingWire</option>
  ...
  return (
    <form onSubmit={submit} className="space-y-8">
      ...
      <div>
        ...
        <select name="time" value={form.time} onChange={handle} required className="input-field bg-transparent">
          <option value="">Select preferred time</option>
          <option>Morning (9 AM – 12 PM)</option>
          <option>Afternoon (12 PM – 3 PM)</option>
          <option>Evening (3 PM – 6 PM)</option>
          <option>Night (6 PM – 9 PM)</option>
          <option>Anytime works</option>
        </select>
      </div>
      <div>
